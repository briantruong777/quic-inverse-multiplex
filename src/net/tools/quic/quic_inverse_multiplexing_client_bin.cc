...
int main(int argc, char* argv[]) {
  ...
  if (line->HasSwitch("port")) {
    if (!base::StringToInt(line->GetSwitchValueASCII("port"), &FLAGS_port) ||
      ...
      std::cerr << "--port and --port2 must be an integer\n";
      return 1;
    }
  }
  ...
  VLOG(1) << "server host: " << FLAGS_host
          << " port: " << FLAGS_port << " port2: " << FLAGS_port2
  ...
  cout << "Connected to " << host_port << " and " << host_port2 << endl;
